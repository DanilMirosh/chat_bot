...
from apps.currency_converter_app import check_valid_currency, check_numeric, exchange_request
from apps.images import get_random_image
...
# Слушаем команду 'прогноз погоды' и записываем ответ в city_state
@rate_limit(limit=5)
...
# Проверяем валидность и записываем первый тикер из пары в from_currency_state, далее слушаем следующий ответ
@rate_limit(limit=5)
...
# Проверяем валидность и записываем второй тикер из пары в to_currency_state, слушаем следующий ответ
@rate_limit(limit=5)
...
# Проверяем валидность и записываем кол-во amount_state.
# Если валидно, то используем exchange_request, чтобы узнать результат
@rate_limit(limit=5)
